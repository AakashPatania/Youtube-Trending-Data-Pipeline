...
## What's actually happening here in below code ??

""" 
We have messy Youtube Data in S3 (bronze)

Glue job cleans it, organizes it,

stores clean data in s3 (silver) + make it queryable
...
# Take input details (arguments)
# where is raw data? and Where to save clean data?
...
glueContext = GlueContext(sc) # Glue Wrapper over spark , Glue uses spark behind the scene
spark = glueContext.spark_session
job = Job(glueContext)
job.init(args['JOB_NAME'], args) # Initializes glue job

logger = glueContext.get_logger() # better than print

# Config

"""
Take input values (arguments and store them in variables for easy use)
...
""" Go to s3 -> Bronze Layer -> get data """
...
# It reads data from S3 using glue table(metadata) and loads it into your job.
...
if initial_count == 0:
    ...
    # handles both csv and json format
